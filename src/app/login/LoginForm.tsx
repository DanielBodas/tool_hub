      ...
      <div className="max-w-md w-full bg-card p-6 sm:p-10 rounded-3xl sm:rounded-[2.5rem] shadow-xl border border-border">
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground mt-2">
            Introduce el código para entrar al panel
          </p>
        </div>

        {error && (
          <div className="mb-6 p-4 bg-destructive/10 text-destructive rounded-xl text-sm font-semibold border border-destructive/20">
            {error}
          </div>
        )}
        ...
        <form onSubmit={handleAdminLogin} className="space-y-6">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-4 bg-primary text-primary-foreground rounded-2xl font-bold hover:bg-primary/95 transition-all shadow-lg active:scale-[0.98] disabled:opacity-50"
          >
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-3 px-4 py-4 border-2 border-border rounded-2xl font-bold text-foreground hover:bg-muted transition-all active:scale-[0.98]"
        >

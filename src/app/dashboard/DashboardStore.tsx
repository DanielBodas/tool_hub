...
export default function DashboardStore({ tools }: DashboardStoreProps) {
  // Use lazy state initialization to avoid calling setState synchronously inside useEffect (avoiding ESLint errors)
  ...
  return (
    <div className="container mx-auto px-4 py-6 max-w-7xl">
      {/* Dynamic Grid Layout: Steam App Store Concept */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">

        {/* LEFT COLUMN: Steam-like Sidebar / Library Selector */}
        <div className="lg:col-span-3 space-y-6">
          ...
          <div className="bg-card/40 backdrop-blur-md rounded-2xl p-5 border border-border/60 shadow-xs">
            ...
            <h2 className="text-xl font-black text-foreground tracking-tight flex items-center gap-1.5">
              <Gamepad2 className="text-primary w-5 h-5" /> SteamHub Store
            </h2>
            <p className="text-xs text-muted-foreground mt-1">
              Organiza, marca como favorito y monitoriza tus herramientas web.
            </p>
          </div>
          ...
          {/* Quick Stats Panel (Steam Library Panel) */}
          <div className="bg-card/20 backdrop-blur-xs rounded-2xl p-4.5 border border-border/40 text-xs space-y-3">
            <h4 className="font-extrabold text-muted-foreground uppercase tracking-widest text-[10px] flex items-center gap-1.5">
              <TrendingUp size={12} className="text-emerald-500" /> Estadísticas del Perfil
            </h4>

            <div className="grid grid-cols-2 gap-3">
              <div className="bg-card/40 p-2.5 rounded-xl border border-border/30">
                <p className="text-[10px] text-muted-foreground font-medium">Uso Total</p>
                <p className="text-base font-black text-foreground mt-0.5">
                  {stats.totalPlays} <span className="text-[10px] font-normal text-muted-foreground">ejec.</span>
                </p>
              </div>
              <div className="bg-card/40 p-2.5 rounded-xl border border-border/30">
            ...
            <div className="bg-card/40 p-3 rounded-xl border border-border/30 space-y-1">
              <div className="flex justify-between text-[11px]">
                <span className="text-muted-foreground">Más ejecutada:</span>
        ...
        {/* RIGHT COLUMN: Featured + Tools Grid / List */}
        <div className="lg:col-span-9 space-y-6">

          {/* HERO BANNER: Steam Featured Tool Showcase */}
          {featuredTool && selectedCategory === "ALL" && !searchQuery && (
            <div className="relative group rounded-3xl overflow-hidden border border-border/60 bg-gradient-to-br from-indigo-950/40 via-card to-background shadow-lg">
                  ...
                  <div className="flex flex-wrap items-center gap-3 pt-1">
                    ...
                    {playCounts[featuredTool.id] > 0 && (
                      <span className="text-[11px] font-bold text-emerald-500 bg-emerald-500/10 px-2 py-0.5 rounded-md flex items-center gap-1">
                        <Flame size={12} /> {playCounts[featuredTool.id]} ejecuciones
                      </span>
                    )}
                  </div>
                </div>

                {/* Right side: huge icon & play action */}
                <div className="flex flex-col sm:flex-row md:flex-col items-center gap-4 w-full md:w-auto shrink-0 border-t md:border-t-0 md:border-l border-border/40 pt-4 md:pt-0 md:pl-8">
                  ...
                  <div className="flex items-center gap-2 w-full sm:w-auto md:w-full justify-center">
                    ...
                    <Link
                      ...
                      className="flex-1 sm:flex-initial md:w-full inline-flex items-center justify-center gap-2 px-6 py-3.5 bg-primary hover:bg-primary-hover text-primary-foreground font-black text-sm rounded-xl shadow-md hover:shadow-primary/20 hover:scale-[1.02] active:scale-[0.98] transition-all"
                    >
                      <Play size={16} fill="currentColor" /> JUGAR / EJECUTAR
                    </Link>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-card/25 backdrop-blur-xs p-4 rounded-2xl border border-border/40">
            ...
            <div className="flex items-center gap-3 shrink-0 self-end sm:self-auto">
              ...
              {/* Sort selector */}
              <div className="flex items-center gap-1.5 bg-card border border-border/80 px-2.5 py-1.5 rounded-xl text-xs font-bold text-muted-foreground">
                <ArrowUpDown size={12} className="text-primary" />
                <select
                  value={sortBy}
                  onChange={(e) => setSortBy(e.target.value as "name" | "plays" | "category")}
                  className="bg-transparent border-0 outline-hidden focus:ring-0 text-foreground text-xs cursor-pointer font-bold"
                >
                  <option value="name" className="bg-card text-foreground">Nombre (A-Z)</option>
                  <option value="plays" className="bg-card text-foreground">Más Abiertos</option>
          ...
          {filteredTools.length > 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4.5">
                {filteredTools.map((tool) => {
                  ...
                  return (
                    <div
                      key={tool.id}
                      className="group relative bg-card hover:bg-muted/30 rounded-2xl border border-border/80 p-4.5 hover:shadow-md hover:border-primary/40 transition-all duration-300 flex flex-col justify-between overflow-hidden"
                    >
                      ...
                      <div>
                        ...
                        <div className="flex justify-between items-start mb-3.5">
                          {/* Colored Icon box */}
                          <div className="w-10 h-10 bg-primary/10 text-primary rounded-xl flex items-center justify-center group-hover:bg-primary group-hover:text-primary-foreground group-hover:scale-105 transition-all duration-200">
                            <ToolIcon size={20} />
                          </div>
                        ...
                        {/* Text info */}
                        <h4 className="text-sm font-extrabold text-foreground mb-1 group-hover:text-primary transition-colors tracking-tight">
                          {tool.name}
                        </h4>
                        <p className="text-muted-foreground text-xs leading-relaxed line-clamp-2 mb-3.5" title={tool.description}>
                          {tool.description}
                        </p>
                      </div>
                      ...
                      <div className="flex items-center justify-between pt-2.5 border-t border-border/40">
                        ...
                        >
                          Ejecutar <Play size={10} fill="currentColor" />
                        </Link>
                      </div>
                    </div>
                  );
              ...
              <div className="space-y-1.5">
                {filteredTools.map((tool) => {
                  ...
                  return (
                    <div
                      key={tool.id}
                      className="group bg-card hover:bg-muted/35 rounded-xl border border-border/60 px-4 py-2 hover:border-primary/30 transition-all flex items-center justify-between gap-4"
                    >
                      ...
                      <div className="flex items-center gap-3 shrink-0">
                        ...
                        {count > 0 ? (
                          <span className="text-[10px] font-bold text-emerald-500 bg-emerald-500/10 px-2 py-0.5 rounded-md flex items-center gap-1">
                            <Flame size={10} /> {count} <span className="hidden sm:inline font-normal text-muted-foreground text-[9px]">ejec.</span>
                          </span>
                        ) : (
                          <span className="hidden sm:inline text-[10px] text-muted-foreground">Nunca ejecutado</span>
                        )}
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          <Link
                            href={tool.href}
                            onClick={() => registerLaunch(tool.id)}
                            className="inline-flex items-center justify-center gap-1 px-3 py-1 bg-primary hover:bg-primary-hover text-primary-foreground font-black text-xs rounded-lg shadow-xs hover:scale-[1.02] active:scale-[0.98] transition-all"
                          >
            ...
            /* EMPTY FALLBACK */
            <div className="flex flex-col items-center justify-center text-center p-12 bg-card/25 rounded-3xl border-2 border-dashed border-border/80">
              <Package className="w-12 h-12 text-muted-foreground/60 mb-4 animate-bounce" />
              <h4 className="font-extrabold text-foreground mb-1 text-sm">
                No se encontraron herramientas
              </h4>
              <p className="text-xs text-muted-foreground max-w-sm leading-normal mb-4">
                No hay herramientas que coincidan con la categoría, favoritos o filtros de búsqueda seleccionados.
              </p>
              <button
                onClick={() => {

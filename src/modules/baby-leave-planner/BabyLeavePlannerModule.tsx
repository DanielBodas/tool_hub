...
export function BabyLeavePlannerModule() {
  ...
  const renderMonth = (date: Date, showHeader = true) => {
    ...
    return (
      <div key={`${date.getFullYear()}-${date.getMonth()}`} className="space-y-4">
        {showHeader && (
          <h3 className="text-center font-bold capitalize text-gray-700 dark:text-gray-300">{date.toLocaleDateString('es-ES', { month: 'long', year: 'numeric' })}</h3>
        )}
        <div className="grid grid-cols-7 gap-1">
          {['L', 'M', 'X', 'J', 'V', 'S', 'D'].map(d => (
            <div key={d} className="text-center text-[8px] font-bold text-gray-400 dark:text-gray-500 uppercase py-1">{d}</div>
          ))}
  ...
  return (
    <div className="relative min-h-screen pb-20 md:pb-0 bg-white dark:bg-gray-950 transition-colors duration-300">
      ...
      <div className="flex justify-between items-center mb-6 px-4 md:px-0">
        ...
        <div className="hidden md:flex items-center gap-2">
          <button
            ...
            className={`p-2 rounded-xl border transition-colors ${leftDrawerOpen ? 'bg-pink-50 dark:bg-pink-900/20 border-pink-200 dark:border-pink-800 text-pink-600' : 'bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-800 text-gray-500 hover:bg-gray-50 dark:hover:bg-gray-800'}`}
            title="Gestión Madre"
          >
          ...
          <button
            ...
            className={`p-2 rounded-xl border transition-colors ${rightDrawerOpen ? 'bg-blue-50 dark:bg-blue-900/20 border-blue-200 dark:border-blue-800 text-blue-600' : 'bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-800 text-gray-500 hover:bg-gray-50 dark:hover:bg-gray-800'}`}
            title="Gestión Padre"
          >
          ...
          <button
            ...
            className={`p-2 rounded-xl border transition-colors ${showSettings ? 'bg-blue-600 text-white border-blue-600' : 'bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-800 text-gray-500 hover:bg-gray-50 dark:hover:bg-gray-800'}`}
            title="Ajustes"
          >
            <Settings size={20} />
          </button>
            ...
            <div className="bg-white dark:bg-gray-900 p-4 md:p-6 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex bg-gray-100 dark:bg-gray-800 p-1 rounded-lg">
                    <button
                      ...
                      className={`px-3 py-1 text-[10px] font-bold rounded-md transition ${viewMode === 'month' ? 'bg-white dark:bg-gray-900 shadow-sm text-blue-600 dark:text-blue-400' : 'text-gray-500 dark:text-gray-400'}`}
                    >
                      MES
                    </button>
                    <button
                      onClick={() => setViewMode('year')}
                      className={`px-3 py-1 text-[10px] font-bold rounded-md transition ${viewMode === 'year' ? 'bg-white dark:bg-gray-900 shadow-sm text-blue-600 dark:text-blue-400' : 'text-gray-500 dark:text-gray-400'}`}
                    >
                      AÑO
                    </button>
                  </div>
                </div>
                <div className="flex items-center gap-2 bg-gray-50 dark:bg-gray-800 p-1 rounded-xl border border-gray-100 dark:border-gray-700 w-full md:w-auto justify-between md:justify-start text-gray-900 dark:text-white">
                  <button onClick={() => changeMonth(viewMode === 'month' ? -1 : -12)} className="p-3 md:p-2 hover:bg-white dark:hover:bg-gray-700 hover:shadow-sm rounded-lg transition bg-white/50 dark:bg-transparent"><ChevronLeft size={24} className="md:w-5 md:h-5 text-gray-500 dark:text-gray-400" /></button>
                  <span className="font-extrabold min-w-[120px] text-center capitalize text-xs md:text-sm">
                    {viewMode === 'month'
                      ? viewDate.toLocaleDateString('es-ES', { month: 'long', year: 'numeric' })
                      : viewDate.getFullYear()}
                  </span>
                  <button onClick={() => changeMonth(viewMode === 'month' ? 1 : 12)} className="p-3 md:p-2 hover:bg-white dark:hover:bg-gray-700 hover:shadow-sm rounded-lg transition bg-white/50 dark:bg-transparent"><ChevronRight size={24} className="md:w-5 md:h-5 text-gray-500 dark:text-gray-400" /></button>
                </div>
                <button
                  onClick={() => { setShowSettings(!showSettings); setMobileTab('calendar'); }}
                  className={`p-2 rounded-xl border transition-colors ${showSettings ? 'bg-blue-600 text-white border-blue-600' : 'bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-800 text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800'}`}
                  title="Configuración"
                >
                  <Settings size={20} />
                </button>
              ...
              {viewMode === 'month' ? (
                <div className="grid grid-cols-7 gap-1 md:gap-2">
                  {['Lun', 'Mar', 'Mié', 'Jue', 'Vie', 'Sáb', 'Dom'].map(d => <div key={d} className="text-center text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase py-2">{d}</div>)}
              ...
              {selectedDay && (
                <div className="fixed inset-x-0 bottom-16 md:relative md:bottom-0 md:mt-6 p-4 md:p-3 bg-white dark:bg-gray-900 border-t md:border border-gray-200 dark:border-gray-800 shadow-2xl md:shadow-none animate-in slide-in-from-bottom md:slide-in-from-top duration-300 z-40 md:rounded-2xl">
                  <div className="flex flex-col gap-4">
                    {/* Header + Actions */}
                    <div className="flex justify-between items-center border-b border-gray-100 dark:border-gray-800 md:border-none pb-2 md:pb-0">
                      <div className="flex items-center gap-3">
                        <span className="text-[10px] font-black bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300 px-2 py-1 rounded-md">{formatDate(selectedDay).split('-').reverse().slice(0,2).join('/')}</span>
                        <div className="flex bg-gray-100 dark:bg-gray-800 p-1 rounded-lg text-white">
                          <button
                            onClick={() => setAssignmentParent('mother')}
                            className={`px-3 py-1 text-[9px] font-black rounded-md transition ${assignmentParent === 'mother' ? 'bg-pink-500 text-white shadow-sm' : 'text-gray-400 dark:text-gray-500'}`}
                          >
                            MAMÁ
                          </button>
                          <button
                            onClick={() => setAssignmentParent('father')}
                            className={`px-3 py-1 text-[9px] font-black rounded-md transition ${assignmentParent === 'father' ? 'bg-blue-500 text-white shadow-sm' : 'text-gray-400 dark:text-gray-500'}`}
                          >
                            PAPÁ
                          </button>
                        </div>
                      </div>
                      <button onClick={() => setSelectedDay(null)} className="text-gray-300 dark:text-gray-600 hover:text-gray-500 dark:hover:text-gray-400 transition">
                        <Trash2 size={18} />
                      </button>
                    </div>
              ...
              <div className="mt-6 flex flex-wrap gap-4 text-[10px] font-bold uppercase tracking-wider justify-center md:justify-start text-gray-500 dark:text-gray-400">
                ...
                <div className="flex items-center gap-2"><div className="w-3 h-3 bg-amber-100 dark:bg-amber-900/40 border border-amber-200 dark:border-amber-800 rounded-sm"></div> Festivo</div>
              </div>
            </div>
          </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white/80 dark:bg-gray-950/80 backdrop-blur-lg border-t border-gray-100 dark:border-gray-800 flex justify-around items-center p-2 z-[60] md:hidden">
        <button
          onClick={() => { setMobileTab('mother'); setShowSettings(false); setLeftDrawerOpen(false); setRightDrawerOpen(false); }}
          className={`flex flex-col items-center gap-1 p-2 flex-1 ${mobileTab === 'mother' ? 'text-pink-600' : 'text-gray-400 dark:text-gray-500'}`}
        >
          <User size={20} />
          <span className="text-[9px] font-bold">MAMÁ</span>
        </button>
        <button
          onClick={() => { setMobileTab('calendar'); setShowSettings(false); setLeftDrawerOpen(false); setRightDrawerOpen(false); }}
          className={`flex flex-col items-center gap-1 p-2 flex-1 ${mobileTab === 'calendar' ? 'text-blue-600' : 'text-gray-400 dark:text-gray-500'}`}
        >
          <CalendarIcon size={20} />
          <span className="text-[9px] font-bold">CALENDARIO</span>
        </button>
        <button
          onClick={() => { setMobileTab('father'); setShowSettings(false); setLeftDrawerOpen(false); setRightDrawerOpen(false); }}
          className={`flex flex-col items-center gap-1 p-2 flex-1 ${mobileTab === 'father' ? 'text-blue-600' : 'text-gray-400 dark:text-gray-500'}`}
        >
          <User size={20} />

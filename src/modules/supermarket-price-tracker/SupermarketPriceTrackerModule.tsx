...
export function SupermarketPriceTrackerModule() {
  ...
  return (
    <div className="space-y-2.5 max-w-7xl mx-auto px-1 sm:px-3 py-1.5 text-foreground min-w-0">

      {/* ULTRA COMPACT MOBILE HEADER BAR */}
      <div className="bg-card/90 backdrop-blur-md rounded-2xl border border-border/80 p-2.5 sm:p-3.5 shadow-2xs flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-1.5 flex-wrap">
              ...
              <span className="text-[9px] sm:text-[10px] font-black uppercase px-1.5 py-0.2 rounded-full bg-emerald-500/10 text-emerald-600 border border-emerald-500/20 shrink-0">
                Ahorro
              </span>
            </div>
            <p className="text-[10px] text-muted-foreground hidden sm:block truncate">
              Verificador de ofertas reales vs engaños por precio unitario (€/kg, €/L, €/ud).
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          <button
            ...
            disabled={isSaving}
            className={`p-1.5 sm:px-2.5 sm:py-1 rounded-xl text-[10px] font-extrabold transition-all flex items-center gap-1 border ${
              dbStatus === "synced"
          ...
          >
            ...
            <span className="hidden sm:inline">{isSaving ? "Guardando..." : dbStatus === "synced" ? "BD" : "Guardar"}</span>
          </button>

          <button
            onClick={handleResetToDefaults}
            className="p-1.5 sm:px-2.5 sm:py-1 bg-muted/60 hover:bg-muted text-muted-foreground hover:text-foreground rounded-xl text-[10px] font-extrabold transition-all border border-border/60 flex items-center gap-1"
      ...
      {/* ULTRA COMPACT MOBILE SEGMENTED CAPSULE TABS */}
      <div className="flex items-center justify-between bg-card p-1 rounded-2xl border border-border/80 shadow-2xs gap-1">
        <button
          onClick={() => setActiveTab("verifier")}
          className={`flex-1 py-1.5 px-1.5 sm:px-3 rounded-xl text-[10px] sm:text-xs font-black transition-all flex items-center justify-center gap-1 min-w-0 ${
            activeTab === "verifier"
        ...
        >
          ...
          <span className="truncate">Histórico ({priceRecords.length})</span>
        </button>
      ...
      {/* ========================================================================= */}
      {/* TAB 1: VERIFICADOR DE OFERTA EN TIEMPO REAL (MOBILE OPTIMIZED) */}
      {/* ========================================================================= */}
      {activeTab === "verifier" && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-3 items-start min-w-0">

          {/* INPUT FORM PANEL */}
          <div className="lg:col-span-5 bg-card rounded-2xl border border-border/80 p-3 sm:p-4 shadow-2xs space-y-3 min-w-0">
            ...
            <div className="space-y-2.5">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full bg-background border border-border/80 rounded-xl px-2.5 py-1.5 text-xs font-bold focus:outline-none focus:ring-2 focus:ring-primary/40 truncate"
                >
                  {products.map((p) => (
              ...
              {/* Supermarket & Brand Grid (2-column on mobile) */}
              ...
              {/* Price & Format Row (Optimized Grid) */}
              <div className="grid grid-cols-12 gap-1.5 pt-0.5">
                <div className="col-span-6">
                  ...
                  <input
                    ...
                    className="w-full bg-background border border-border/80 rounded-xl px-2.5 py-1.5 text-xs font-black focus:outline-none focus:ring-2 focus:ring-primary/40"
                  />
                </div>
                ...
                <div className="col-span-3">
                  ...
                  <input
                    ...
                    className="w-full bg-background border border-border/80 rounded-xl px-2 py-1.5 text-xs font-bold focus:outline-none focus:ring-2 focus:ring-primary/40 text-center"
                  />
                </div>
                ...
                <div className="col-span-3">
                  ...
                  <select
                    ...
                    className="w-full bg-background border border-border/80 rounded-xl px-1 py-1.5 text-xs font-bold focus:outline-none focus:ring-2 focus:ring-primary/40 text-center"
                  >
          ...
          <div className="lg:col-span-7 space-y-3 min-w-0">
            {currentOfferAssessment ? (
              <div
                className={`rounded-2xl border p-3.5 sm:p-4 transition-all shadow-2xs ${
              ...
              >
                {/* STATUS BADGE HEADER */}
                <div className="flex flex-wrap items-center justify-between gap-1.5 border-b border-current/10 pb-2.5 mb-2.5">
                  <div className="flex items-center gap-2 min-w-0">
                    {currentOfferAssessment.status === "CHOLLO" && (
                    ...
                    <div className="min-w-0">
                      <span className="text-[9px] font-black uppercase tracking-wider opacity-80 block">
                        DIAGNÓSTICO DE PRECIO
                      </span>
                      <h2 className="text-xs sm:text-base font-black leading-tight truncate">
                        {currentOfferAssessment.status === "CHOLLO" && "🟢 CHOLLO REAL / EXCELENTE"}
                  ...
                  <span className="text-sm sm:text-lg font-black px-2.5 py-0.5 rounded-xl bg-background/80 shadow-2xs border border-current/20 shrink-0">
                    {currentOfferAssessment.currentUnitPrice.toFixed(2)} {currentOfferAssessment.baseUnitLabel}
                  </span>
                </div>

                {/* VERDICT MESSAGE */}
                <p className="text-[11px] sm:text-xs font-semibold mb-3 leading-relaxed">
                ...
                {currentOfferAssessment.status !== "SIN_DATOS" && (
                  <div className="grid grid-cols-3 gap-1.5 pt-1">
                    <div className="bg-background/80 backdrop-blur-xs rounded-xl p-1.5 border border-current/15 text-center min-w-0">
                      <span className="text-[8px] font-extrabold uppercase text-muted-foreground block truncate">
                        MÍNIMO
                      </span>
                      <span className="text-[11px] sm:text-xs font-black text-emerald-600 dark:text-emerald-400 truncate block">
                    ...
                    <div className="bg-background/80 backdrop-blur-xs rounded-xl p-1.5 border border-current/15 text-center min-w-0">
                      ...
                      <span className="text-[11px] sm:text-xs font-black text-amber-600 dark:text-amber-400 truncate block">
                    ...
                    <div className="bg-background/80 backdrop-blur-xs rounded-xl p-1.5 border border-current/15 text-center min-w-0">
                      ...
                      <span className="text-[11px] sm:text-xs font-black text-rose-600 dark:text-rose-400 truncate block">
              ...
              <div className="bg-card rounded-2xl border border-dashed border-border p-6 text-center flex flex-col items-center justify-center space-y-1.5">
                <ShoppingCart className="w-8 h-8 text-muted-foreground/40 animate-pulse" />
                <h4 className="font-extrabold text-xs text-foreground">
                  Introduce un precio para evaluar la oferta
                </h4>
                <p className="text-[10px] text-muted-foreground max-w-xs">
                  Selecciona un producto e ingresa el precio total y la cantidad para comprobar si estás ahorrando o pagando más.
                </p>
              </div>
            )}
            ...
            {verifierProductId && (
              <div className="bg-card rounded-2xl border border-border/80 p-3 shadow-2xs space-y-2 min-w-0">
                <h4 className="text-[10px] font-black text-foreground flex items-center gap-1 uppercase tracking-wider">
                  <History className="w-3 h-3 text-primary" /> Registros Históricos Recientes
                </h4>
                  ...
                  {priceRecords
                    ...
                    .map((rec) => {
                      ...
                      return (
                        <div
                          key={rec.id}
                          className="flex items-center justify-between gap-1.5 bg-muted/20 hover:bg-muted/40 p-1.5 rounded-xl border border-border/50 text-[11px] transition-all min-w-0"
                        >
                          ...
                          <div className="text-right shrink-0">
                            <span className="font-black text-foreground block text-[11px]">
      ...
      {activeTab === "history" && (
        <div className="space-y-3 min-w-0">
          ...
          <div className="bg-card rounded-2xl border border-border/80 p-3 shadow-2xs space-y-2.5">
            <div className="flex items-center justify-between gap-2 border-b border-border/50 pb-2">
              <button
                onClick={() => setShowMobileFilters(!showMobileFilters)}
                className="text-xs font-black text-foreground flex items-center gap-1.5"
              >
            ...
            {(showMobileFilters || activeFilterCount > 0) && (
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-2 pt-1 border-t border-border/40">
                {/* Product */}
                <select
                  value={historyProductId}
                  onChange={(e) => setHistoryProductId(e.target.value)}
                  className="bg-background border border-border/80 rounded-xl px-2 py-1.5 text-xs font-bold focus:outline-none focus:ring-2 focus:ring-primary/40 truncate"
                >
                ...
                {/* Offers toggle */}
                <label className="col-span-2 sm:col-span-1 flex items-center justify-center gap-1.5 bg-background border border-border/80 rounded-xl px-2 py-1.5 cursor-pointer text-xs font-bold select-none hover:bg-muted/30">
                  <input
                    type="checkbox"
          ...
          <div className="bg-card rounded-2xl border border-border/80 overflow-hidden shadow-2xs">
            <div className="p-2.5 bg-muted/20 border-b border-border/60 flex items-center justify-between">
              ...
              <div className="divide-y divide-border/60">
                {filteredHistory.map((rec) => {
                  ...
                  return (
                    <div
                      key={rec.id}
                      className="p-2.5 hover:bg-muted/30 transition-all flex items-center justify-between gap-2 min-w-0"
                    >
                      <div className="flex items-center gap-2 min-w-0 flex-1">
                        ...
                        <div className="space-y-0.5 min-w-0 flex-1">
                          ...
                          <div className="text-[10px] text-muted-foreground flex items-center gap-1.5 flex-wrap font-medium">
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        <div className="text-right">
                          ...
                          <div className="text-[9px] sm:text-[10px] font-extrabold text-primary">
      ...
      {/* ========================================================================= */}
      {/* TAB 3: REGISTRAR UN NUEVO PRECIO (MOBILE OPTIMIZED) */}
      {/* ========================================================================= */}
      {activeTab === "add" && (
        <div className="max-w-xl mx-auto bg-card rounded-2xl border border-border/80 p-3.5 sm:p-5 shadow-2xs space-y-3 min-w-0">
          <div className="border-b border-border/60 pb-2">
            <h3 className="text-sm font-black text-foreground flex items-center gap-1.5">
              <PlusCircle className="w-4 h-4 text-primary" /> Registrar Nuevo Precio
            </h3>
          </div>

          <form onSubmit={handleAddPriceRecord} className="space-y-2.5">
            {/* Product Select */}
            <div>
              <label className="block text-[10px] font-extrabold uppercase tracking-wider text-muted-foreground mb-1">
                Producto *
            ...
            {/* Date & Price Row (2-column on mobile) */}
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="block text-[10px] font-extrabold uppercase tracking-wider text-muted-foreground mb-1">
                  Fecha *
                ...
                <input
                  ...
                  required
                  className="w-full bg-background border border-border/80 rounded-xl px-2 py-1.5 text-xs font-bold focus:outline-none focus:ring-2 focus:ring-primary/40"
                />
              </div>
            ...
            {/* Quantity & Unit Row (2-column on mobile) */}
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="block text-[10px] font-extrabold uppercase tracking-wider text-muted-foreground mb-1">
                  Cantidad
                ...
                <input
                  ...
                  className="w-full bg-background border border-border/80 rounded-xl px-2.5 py-1.5 text-xs font-bold focus:outline-none focus:ring-2 focus:ring-primary/40"
                />
              </div>
            ...
            {/* Offer details */}
            <div className="bg-muted/20 rounded-xl p-2.5 border border-border/60 space-y-1.5">
              <label className="flex items-center gap-1.5 cursor-pointer text-xs font-bold select-none">
                <input
                  ...
                  className="rounded text-primary focus:ring-primary"
                />
                <span>🏷️ Marca como Precio de Oferta</span>
              </label>
              ...
              {addIsOffer && (
                <input
                  ...
                  className="w-full bg-background border border-border/80 rounded-xl px-2.5 py-1 text-xs font-bold focus:outline-none focus:ring-2 focus:ring-primary/40"
                />
              )}
            </div>
            ...
            <button
              type="submit"
              className="w-full py-2 bg-primary hover:bg-primary-hover text-primary-foreground font-black text-xs rounded-xl shadow-xs transition-all flex items-center justify-center gap-1.5"
            >
              <PlusCircle size={14} /> Guardar Precio
            </button>
          </form>
        </div>
      )}
      ...
      {activeTab === "settings" && (
        <div className="space-y-3 min-w-0">
          ...
          <div className="flex items-center gap-1 bg-card p-1 rounded-xl border border-border/80 w-full sm:w-fit overflow-x-auto">
            <button
              onClick={() => setSettingsSection("brands")}
              className={`flex-1 sm:flex-initial px-2.5 py-1.5 rounded-lg text-[11px] font-bold transition-all flex items-center justify-center gap-1 shrink-0 ${
            ...
            >
              <Store size={12} /> Supers ({supermarkets.length})
            </button>
          ...
          {settingsSection === "brands" && (
            <div className="bg-card rounded-2xl border border-border/80 p-3 sm:p-4 shadow-2xs space-y-3 min-w-0">
              <div className="flex items-center justify-between gap-2 border-b border-border/60 pb-2">
                ...
                <button
                  ...
                  className="px-2.5 py-1 bg-primary text-primary-foreground rounded-xl text-[10px] sm:text-xs font-bold transition-all flex items-center gap-1 shrink-0"
                >
                  <PlusCircle size={12} /> Nueva Marca
                </button>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                {brands.map((b) => {
                  ...
                  return (
                    <div
                      key={b.id}
                      className="bg-muted/20 hover:bg-muted/40 rounded-2xl border border-border/60 p-2.5 flex flex-col justify-between gap-1.5 transition-all min-w-0"
                    >
            ...
            <div className="bg-card rounded-2xl border border-border/80 p-3 sm:p-4 shadow-2xs space-y-3 min-w-0">
              <div className="flex items-center justify-between gap-2 border-b border-border/60 pb-2">
                ...
                <button
                  ...
                  className="px-2.5 py-1 bg-primary text-primary-foreground rounded-xl text-[10px] sm:text-xs font-bold transition-all flex items-center gap-1 shrink-0"
                >
                  <PlusCircle size={12} /> Nuevo Super
                </button>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                {supermarkets.map((sm) => (
                  <div
                    key={sm.id}
                    className="bg-muted/20 hover:bg-muted/40 rounded-2xl border border-border/60 p-2.5 flex items-center justify-between gap-1.5 min-w-0"
                  >
            ...
            <div className="bg-card rounded-2xl border border-border/80 p-3 sm:p-4 shadow-2xs space-y-3 min-w-0">
              <div className="flex items-center justify-between gap-2 border-b border-border/60 pb-2">
                ...
                <button
                  ...
                  className="px-2.5 py-1 bg-primary text-primary-foreground rounded-xl text-[10px] sm:text-xs font-bold transition-all flex items-center gap-1 shrink-0"
                >
                  <PlusCircle size={12} /> Nuevo Producto
                </button>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2">
                {products.map((p) => (
                  <div
                    key={p.id}
                    className="bg-muted/20 hover:bg-muted/40 rounded-2xl border border-border/60 p-2.5 flex flex-col justify-between gap-1.5 min-w-0"
                  >
                    <div>
      ...
      {editingBrand && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-2.5 [touch-action:pan-y]">
          <div className="bg-card border border-border rounded-2xl p-4 max-w-md w-full shadow-2xl space-y-3 max-h-[85dvh] overflow-y-auto min-w-0">
            ...
            <form onSubmit={handleSaveBrand} className="space-y-2.5">
              <div>
                ...
                <input
                  ...
                  required
                  className="w-full bg-background border border-border rounded-xl px-2.5 py-1.5 text-xs font-bold"
                />
              </div>

              {/* Supermarket Multi-Select Toggles */}
              <div>
                <label className="block text-[10px] font-extrabold uppercase text-muted-foreground mb-0.5">
                  Exclusividad de Supermercado
                </label>
                <p className="text-[9px] text-muted-foreground mb-1">
                  Selecciona dónde se vende (si no marcas ninguno, en todos).
                </p>
                ...
                <div className="space-y-1">
                  {supermarkets.map((sm) => {
                    ...
                    return (
                      <label
                        key={sm.id}
                        className="flex items-center justify-between p-1.5 rounded-xl border border-border/60 bg-muted/20 cursor-pointer text-xs font-bold hover:bg-muted/40"
                      >
                        <div className="flex items-center gap-1.5 min-w-0">
                          <span
              ...
              <div>
                ...
                <div className="space-y-1 bg-muted/10 p-1.5 rounded-xl border border-border/60 max-h-32 overflow-y-auto">
                  {products.map((p) => {
      ...
      {editingSupermarket && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-2.5 [touch-action:pan-y]">
          <div className="bg-card border border-border rounded-2xl p-4 max-w-xs w-full shadow-2xl space-y-3 min-w-0">
            <div className="flex items-center justify-between border-b border-border/60 pb-2">
              <h3 className="font-black text-xs sm:text-sm text-foreground">
            ...
            <form onSubmit={handleSaveSupermarket} className="space-y-2.5">
              <div>
                <label className="block text-[10px] font-extrabold uppercase text-muted-foreground mb-1">
                  Nombre *
                </label>
